...
export default async function AdminPage() {
  ...
  return <main className="admin-page"><header className="admin-header"><div><Trophy/><span><b>PTSC Sports Festival</b><small>Admin / Quản trị</small></span></div><form action={logout}><SubmitButton><LogOut size={16}/>Đăng xuất</SubmitButton></form></header><div className="admin-layout"><aside><a href="#content">Nội dung / Content</a><a href="#media">Hình ảnh / Media</a><a href="#data">Dữ liệu thi đấu / Competition</a></aside><div className="admin-main">
    <div className="admin-title"><div><h1>Dashboard</h1><p>Xin chào, {admin.display_name}</p></div><a href="/" target="_blank">Xem website ↗</a></div>
    <section id="content" className="admin-card"><h2>Nội dung sự kiện / Event content</h2><form action={saveEvent}><div className="admin-fields"><label><span>Tên sự kiện VI</span><input name="event_name_vi" defaultValue={String(event.event_name_vi ?? "")}/></label><label><span>Event name EN</span><input name="event_name_en" defaultValue={String(event.event_name_en ?? "")}/></label><label><span>Phụ đề VI</span><input name="subtitle_vi" defaultValue={String(event.subtitle_vi ?? "")}/></label><label><span>Subtitle EN</span><input name="subtitle_en" defaultValue={String(event.subtitle_en ?? "")}/></label><label><span>Giới thiệu VI</span><MarkdownInput name="about_vi" defaultValue={String(event.about_vi ?? "")}/></label><label><span>About EN</span><MarkdownInput name="about_en" defaultValue={String(event.about_en ?? "")}/></label><label><span>Địa điểm VI</span><input name="venue_vi" defaultValue={String(event.venue_vi ?? "")}/></label><label><span>Venue EN</span><input name="venue_en" defaultValue={String(event.venue_en ?? "")}/></label><label><span>Bắt đầu</span><input type="datetime-local" name="start_at" defaultValue={event.start_at ? toVietnamLocalInput(String(event.start_at)) : ""}/></label><label><span>Kết thúc</span><input type="datetime-local" name="end_at" defaultValue={event.end_at ? toVietnamLocalInput(String(event.end_at)) : ""}/></label></div><SubmitButton className="gold-button"><Save size={15}/>Lưu nội dung / Save</SubmitButton></form><div className="hero-upload-grid">{([['desktop','KV desktop',String(event.hero_path ?? '')],['mobile','KV mobile',String(event.hero_mobile_path ?? '')]] as const).map(([variant,label,path]) => <form action={uploadHero} key={variant} className="hero-upload"><b>{label}</b><div className="hero-upload-preview">{path ? <Image src={path} alt={label} fill sizes="320px"/> : <span>Chưa có KV</span>}</div><input type="hidden" name="variant" value={variant}/><input type="file" name="file" accept="image/png,image/jpeg,image/webp" required/><SubmitButton className="gold-button">Tải lên / Upload</SubmitButton></form>)}</div></section>
    <section id="media" className="admin-card"><h2><ImagePlus size={18}/> Upload hình ảnh / Media upload</h2><form action={uploadMedia} className="upload-form"><input type="file" name="file" accept="image/png,image/jpeg,image/webp" required/><input name="title_vi" placeholder="Tiêu đề VI"/><input name="title_en" placeholder="Title EN"/><input name="alt_vi" placeholder="Alt VI"/><input name="alt_en" placeholder="Alt EN"/><input name="filter_tag" placeholder="Filter: pickleball, bong-ban…" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" defaultValue="all"/><SubmitButton className="gold-button">Upload</SubmitButton></form></section>
    <section id="sports"><h2 className="section-label">Môn thể thao / Sports</h2><div className="admin-sport-grid">{rows.sports.filter((sport) => !sport.archived_at).map((sport) => <Link key={sport.id} href={`/admin/sports/${sport.slug}`}><b>{summary(sport)}</b><small>Hạng mục, đội, lịch, kết quả, thư viện</small></Link>)}</div></section>
    <section id="data"><h2 className="section-label">Dữ liệu dùng chung / Shared data</h2>{(["organizations", "contacts", "footer_links"] as const).map((entity) => <CrudSection key={entity} entity={entity} rows={rows[entity]} allRows={rows}/>)}</section>
  </div></div></main>;
}
