  ...
  return <main className="admin-page"><header className="admin-header"><div><b>{summary(sport)}</b><small>Quản lý môn / Sport operations</small></div><Link href="/admin"><ArrowLeft size={16}/> Dashboard</Link></header><div className="admin-main admin-sport-main">
    <div className="admin-sport-workspace">
      ...
      <div className="admin-sport-panels">
        ...
        <section id="gallery" className="admin-card"><h2>Thư viện ảnh / Gallery</h2><p className="upload-help">Chọn nhiều ảnh cùng lúc. PNG, JPEG, WebP; tối đa 2MB mỗi ảnh.</p><MediaUploadForm action={uploadMedia} sportId={sport.id} multiple albumFields/><CrudSection entity="media" rows={scopedRows.media} allRows={scopedRows} section="gallery" editTarget={editTarget}/></section>
      </div>
    </div>
  </div></main>;
}
