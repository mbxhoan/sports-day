...
export default async function SportAdminPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="admin-page"><header className="admin-header"><div><b>{summary(sport)}</b><small>Quản lý môn / Sport operations</small></div><Link href="/admin"><ArrowLeft size={16}/> Dashboard</Link></header><div className="admin-main admin-sport-main">
    <div className="admin-sport-workspace">
      <nav className="admin-sport-nav" aria-label="Sport sections"><b>{summary(sport)}</b><small>Danh mục dữ liệu</small><a href="#overview">Thông tin môn</a><a href="#categories">Hạng mục <small>{scopedRows.tournaments.length}</small></a><a href="#teams">Đội & VĐV <small>{scopedRows.entries.length + scopedRows.participants.length}</small></a><a href="#schedule">Lịch & trận <small>{scopedRows.fixtures.length}</small></a><a href="#results">Kết quả</a><a href="#gallery">Thư viện <small>{scopedRows.media.length}</small></a></nav>
      <div className="admin-sport-panels">
        ...
        <section id="teams" className="admin-group"><CrudSection entity="entries" rows={scopedRows.entries} allRows={scopedRows}/><CrudSection entity="participants" rows={scopedRows.participants} allRows={scopedRows}/><CrudSection entity="entry_members" rows={scopedRows.entry_members} allRows={scopedRows}/></section>
