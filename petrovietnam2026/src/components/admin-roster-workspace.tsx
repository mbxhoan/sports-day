...
import { Archive, Check, ClipboardList, Pencil, Plus, Save, Search, Trash2, UserPlus, Users } from "lucide-react";
...
function SearchableSelect({ name: fieldName, value = "", options, placeholder, required = false }: { name: string; value?: string; options: Array<{ value: string; label: string }>; placeholder: string; required?: boolean }) {
  const [query, setQuery] = useState("");
  const selected = options.find((option) => option.value === value);
  const visible = options.filter((option) => option.label.toLowerCase().includes(query.toLowerCase()) || option.value === value);
  return <div className="searchable-select"><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Lọc ${placeholder.toLowerCase()}...`} aria-label={`Lọc ${placeholder.toLowerCase()}`}/><select name={fieldName} defaultValue={value} required={required}><option value="">{placeholder}</option>{selected && !visible.some((option) => option.value === selected.value) && <option value={selected.value}>{selected.label}</option>}{visible.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}</select></div>;
}
    ...
    <div className="roster-grid">
      ...
      <section className="roster-detail-panel">
        {mode === "entries" && selectedEntry ? <>
          ...
          <details className="roster-edit-card" key={selectedEntry.id} open><summary><Pencil size={15}/><span><b>Sửa thông tin đội / cặp</b><small>Cập nhật tên, hạng mục và đơn vị</small></span></summary><form action={saveRecord}><input type="hidden" name="entity" value="entries"/><input type="hidden" name="id" value={selectedEntry.id}/><div className="roster-form-grid"><label><span>Hạng mục</span><SearchableSelect name="tournament_id" value={text(selectedEntry, "tournament_id")} options={tournaments.filter((row) => !row.archived_at).map((row) => ({ value: row.id, label: name(row) }))} placeholder="Chọn hạng mục"/></label><label><span>Loại</span><select name="kind" defaultValue={text(selectedEntry, "kind")}><option value="individual">Cá nhân</option><option value="pair">Cặp</option><option value="team">Đội</option></select></label><label className="field-span-2"><span>Tên hiển thị</span><input name="name_vi" defaultValue={text(selectedEntry, "name_vi")} required/></label><label><span>Tên tiếng Anh</span><input name="name_en" defaultValue={text(selectedEntry, "name_en")}/></label><label><span>Đơn vị</span><SearchableSelect name="organization_id" value={text(selectedEntry, "organization_id")} options={activeOrganizations.map((row) => ({ value: row.id, label: name(row) }))} placeholder="Chưa chọn đơn vị"/></label><label><span>Seed / Số áo</span><input name="seed_number" type="number" min="1" defaultValue={text(selectedEntry, "seed_number")}/></label></div><button className="gold-button"><Save size={16}/>Lưu thay đổi</button></form></details>
          <div className="roster-members-card"><div className="roster-card-head"><div><span className="roster-card-label">DANH SÁCH THÀNH VIÊN</span><h4>Gán VĐV vào đội / cặp</h4><p>Chỉ chọn VĐV có sẵn, không tạo bản ghi trùng.</p></div><strong>{entryMembers.length}</strong></div><form action={saveRecord} className="assign-member-form"><input type="hidden" name="entity" value="entry_members"/><input type="hidden" name="entry_id" value={selectedEntry.id}/><input type="hidden" name="role_en" value="Athlete"/><input type="hidden" name="sort_order" value="0"/><label><span>Lọc danh sách VĐV</span><input value={memberQuery} onChange={(event) => setMemberQuery(event.target.value)} placeholder="Nhập tên hoặc đơn vị..."/></label><label><span>Chọn VĐV</span><SearchableSelect name="participant_id" options={availableParticipants.map((row) => ({ value: row.id, label: `${name(row)} · ${organizationName(row, organizations)}` }))} placeholder="Chọn VĐV" required/></label><label><span>Vai trò</span><input name="role_vi" defaultValue="Vận động viên"/></label><button className="gold-button"><UserPlus size={16}/>Gán vào đội</button></form><div className="member-list">{entryMembers.map((member) => { const participant = activeParticipants.find((row) => row.id === text(member, "participant_id")); return <div className="member-row" key={member.id}><span><b>{participant ? name(participant) : "VĐV không còn hoạt động"}</b><small>{participant ? organizationName(participant, organizations) : ""} · {text(member, "role_vi") || "Vận động viên"}</small></span><form action={setArchived}><input type="hidden" name="entity" value="entry_members"/><input type="hidden" name="id" value={member.id}/><input type="hidden" name="archived" value="true"/><button className="icon-button danger" aria-label={`Bỏ ${participant ? name(participant) : "VĐV"} khỏi đội`} title="Bỏ khỏi đội"><Trash2 size={16}/></button></form></div>; })}{!entryMembers.length && <p className="roster-empty">Đội/cặp này chưa có VĐV. Dùng biểu mẫu phía trên để gán.</p>}</div></div>
