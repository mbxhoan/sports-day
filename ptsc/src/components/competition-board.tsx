...
import { entryDisplayName, formatMatchResult, isReserveRole, normalizeLegacyMatchResult, organizationShortName, scoresFromMatchResult, standingDifference } from "@/lib/competition-display";
...
function isManualSeedSlot(fixture: Pick<Fixture, "round_order" | "bracket_position">, slot: Pick<FixtureSlot, "label_vi" | "label_en">) {
  const label = slot.label_en.trim() || slot.label_vi.trim();
  return fixture.round_order === 1 && fixture.bracket_position !== null && /^seed\s+\d+$/i.test(label);
}

function SlotEditor({ slot, fixture, entries, groups, groupEntries, fixtures, action, sportSlug }: { slot: FixtureSlot; fixture: Fixture; entries: Entry[]; groups: Group[]; groupEntries: GroupEntry[]; fixtures: Fixture[]; action: AdminAction; sportSlug?: string }) {
  ...
  return <form className="slot-source-form" action={formAction}>
    ...
    <label><span>Nguồn</span><SearchableSelect name="source_kind" value={sourceKind} onChange={(value) => setSourceKind(value as FixtureSlot["source_kind"])} options={[{ value: "entry", label: "Đội/cặp/cá nhân" }, { value: "group_rank", label: "Thứ hạng bảng" }, { value: "fixture_winner", label: "Thắng trận" }, { value: "fixture_loser", label: "Thua trận" }, { value: "bye", label: "Bye" }]} placeholder="Chọn nguồn"/></label>
    ...
    {sourceKind === "group_rank" && <><label><span>Bảng nguồn</span><SearchableSelect name="source_group_id" value={groupId} onChange={setGroupId} options={sourceGroups.map((group) => ({ value: group.id, label: group.name_vi }))} placeholder="Chọn bảng"/></label><label><span>Hạng</span><input name="source_rank" type="number" min="1" defaultValue={slot.source_rank ?? ""}/></label>{selectedMembers.length > 0 && <small className="slot-candidates">Thành viên: {selectedMembers.join(" · ")}</small>}</>}
    {(["fixture_winner", "fixture_loser"] as const).includes(sourceKind as "fixture_winner" | "fixture_loser") && <label><span>Trận nguồn</span><SearchableSelect name="source_fixture_id" value={slot.source_fixture_id ?? ""} options={sourceFixtures.map((item) => ({ value: item.id, label: (item.source_code ?? item.round_vi) || item.id }))} placeholder="Chọn trận"/></label>}
...
function SeedBracketSlotEditor({ slot, options, action, sportSlug, locked = false }: { slot: FixtureSlot; options: readonly { value: string; label: string }[]; action: AdminAction; sportSlug?: string; locked?: boolean }) {
  ...
  return <form className="bracket-seed-editor" action={formAction}>
    <input type="hidden" name="slot_id" value={slot.id}/>
    {sportSlug && <input type="hidden" name="sport_slug" value={sportSlug}/>}
    <input type="hidden" name="source_kind" value={entryId ? "entry" : "bye"}/>
    <input type="hidden" name="source_entry_id" value={entryId}/>
    <input type="hidden" name="label_vi" value={slot.label_vi}/>
    <input type="hidden" name="label_en" value={slot.label_en}/>
    <span title={slot.label_vi || slot.label_en}>{slot.label_vi || slot.label_en}</span>
    <SearchableSelect value={entryId} onChange={setEntryId} options={options} placeholder={locked ? "Bracket đã khóa" : options.length ? "Chọn đội" : "Chưa có đội"} disabled={locked || pending || !options.length} aria-label={`Chọn đội cho ${slot.label_vi || slot.label_en}`}/>
    <button type="submit" className="gold-button" disabled={locked || pending || !dirty} aria-label={`Lưu ${slot.label_vi || slot.label_en}`} title={locked ? "Bracket đã có kết quả" : pending ? "Đang lưu..." : "Lưu đội"}>{pending ? "..." : "Lưu"}</button>
    {locked && <small className="slot-candidates">Bracket đã có kết quả; không thể đổi seed.</small>}
    {!state.ok && <small className="form-error" role="alert">{state.message}</small>}
  </form>;
}

function InlineBracketResult({ fixture, rows, action, manualWinner, sportSlug }: { fixture: Fixture; rows: MatchRow[]; action: AdminAction; manualWinner: boolean; sportSlug?: string }) {
  const [state, formAction, pending] = useActionState(action, initialAdminActionState);
  ...
  return <form className="bracket-inline-result" action={formAction}>
    ...
    {manualWinner && <label className="bracket-inline-winner"><span>Thắng</span><SearchableSelect name="winner_entry_id" aria-label="Đội thắng" required={ready} disabled={!ready} value={fixture.winner_entry_id ?? winnerFromScores(scoreValue(rows[0]?.row), scoreValue(rows[1]?.row), rows[0]?.entry?.id ?? "", rows[1]?.entry?.id ?? "")} options={rows.filter((item) => item.entry).map((item) => ({ value: item.entry!.id, label: item.label }))} placeholder="Chọn đội"/></label>}
...
export function CompetitionBoard({ locale, tournaments, entries, groups, groupEntries, fixtures, fixtureEntries, fixtureSlots, standings, adminHref, resultAction, slotAction, previewAction, standingsAction, sportSlug, showTournamentSelector = true, organizations = [], participants = [], entryMembers = [] }: Props) {
  ...
  useEffect(() => {
    if (selectedTournamentId !== "all" && !available.some((tournament) => tournament.id === selectedTournamentId)) setSelectedTournamentId("all");
  ...
  const entryName = (id: string | null | undefined) => id ? entryDisplayName(entriesById.get(id), membersByEntryId.get(id), participants, locale) : "";
  ...
  const matchRows = (fixture: Fixture): MatchRow[] => {
    ...
    return (["home", "away"] as const).map((side, index) => {
      ...
      return { row, entry, slot, candidates, label: entry ? entryName(entry.id) : slot ? slotDisplayLabel(slot) : t.teamsNotAssigned, organizationLabel: entryOrganizationLabel(entry), candidateLabel: !entry && candidates.length ? slotCandidateLabel(candidates, locale) : "" };
    });
  };
  ...
  const tournamentBracketHasResults = (tournamentId: string) => {
    return (fixturesByTournament.get(tournamentId) ?? [])
      .filter((fixture) => fixture.bracket_position !== null)
      .some((fixture) => ["live", "completed"].includes(fixture.status) || Boolean(fixture.winner_entry_id) || (fixtureRows.get(fixture.id) ?? []).some((row) => row.score !== null || row.score_numeric !== null || row.result_status !== null));
  };
  ...
  return <div className="competition-board">{showTournamentSelector && available.length > 1 && <div className="board-selector"><label>Lọc hạng mục / Category<SearchableSelect id="board-tournament" value={selectedTournamentId} onChange={setSelectedTournamentId} options={available.map((tournament) => ({ value: tournament.id, label: localized(tournament, "name", locale) }))} placeholder={t.filterCategory}/></label></div>}{resultAction && <aside className="bracket-help" aria-label="Hướng dẫn cập nhật kết quả"><b>Cách đọc và cập nhật</b><span>Đọc từ trái sang phải. Đường nối đưa đội thắng sang vòng kế tiếp.</span><span>Chọn trận, nhập kết quả hai đội, rồi bấm <strong>Lưu điểm</strong>.</span></aside>}<div className="tournament-stack">{selectedTournaments.map((tournament) => {
    ...
    return <section className="tournament-block" id={`tournament-${tournament.id}`} key={tournament.id} style={{ contentVisibility: "auto", containIntrinsicSize: "0 560px" }}>
      ...
      {isBracket && knockout.length > 0 && (() => {
        ...
        return <ZoomableBracket key={`${tournament.id}-${layout.width}`} locale={locale} layout={layout}><div className="bracket-round-headings">{Array.from({ length: roundCount }, (_, index) => <div className="bracket-round-heading" key={index} style={{ left: index * (CARD_WIDTH + COLUMN_GAP), width: CARD_WIDTH }}>{roundLabels.get(index + 1) ?? `${locale === "vi" ? "Vòng" : "Round"} ${index + 1}`}</div>)}</div><svg viewBox={`0 0 ${layout.width} ${layout.height}`} aria-hidden="true">{layout.connectors.map((connector) => <path key={`${connector.sourceId}-${connector.targetId}`} d={connector.path}/>)}</svg>{layout.nodes.map((node) => {
          const fixture = fixturesById.get(node.id)!;
          const rows = matchRows(fixture);
          const seedRows = (fixtureSlotsById.get(fixture.id) ?? []).filter((slot) => isManualSeedSlot(fixture, slot));
          const hasSeedEditors = Boolean(resultAction && slotAction && seedRows.length > 0);
          const card = <article className={`bracket-match source-bracket-match${resultAction ? " admin-bracket-match" : ""}${hasSeedEditors ? " has-seed-editors" : ""}`}>
