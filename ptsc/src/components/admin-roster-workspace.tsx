    ...
    <div className="roster-grid">
      ...
      <section className="roster-detail-panel">
        {mode === "entries" && selectedEntry ? <>
          ...
          <div className="roster-members-card"><div className="roster-card-head"><div><span className="roster-card-label">DANH SÁCH THÀNH VIÊN</span><h4>Gán VĐV vào đội / cặp</h4><p>Chỉ chọn VĐV có sẵn, không tạo bản ghi trùng.</p></div><strong>{entryMembers.length}</strong></div><RosterActionForm action={saveRosterRecord} sportSlug={sportSlug} className="assign-member-form" submitLabel="Gán vào đội" pendingLabel="Đang gán..."><input type="hidden" name="entity" value="entry_members"/><input type="hidden" name="entry_id" value={selectedEntry.id}/><input type="hidden" name="role_en" value="Athlete"/><input type="hidden" name="sort_order" value="0"/><label><span>Lọc danh sách VĐV</span><input value={memberQuery} onChange={(event) => setMemberQuery(event.target.value)} placeholder="Nhập tên hoặc đơn vị..."/></label><label><span>Chọn VĐV</span><SearchableSelect name="participant_id" options={availableParticipants.map((row) => ({ value: row.id, label: `${name(row)} · ${organizationName(row, organizations)}` }))} placeholder="Chọn VĐV" required/></label><label><span>Vai trò</span><input name="role_vi" defaultValue="Vận động viên"/></label></RosterActionForm><div className="member-list">{entryMembers.map((member) => { const participant = activeParticipants.find((row) => row.id === text(member, "participant_id")); return <div className="member-row" key={member.id}><span><b>{participant ? name(participant) : "VĐV không còn hoạt động"}</b><small>{participant ? organizationName(participant, organizations) : ""} · {text(member, "role_vi") || "Vận động viên"}</small></span><form action={setArchived}><input type="hidden" name="entity" value="entry_members"/><input type="hidden" name="id" value={member.id}/><input type="hidden" name="archived" value="true"/><button className="icon-button danger" aria-label={`Bỏ ${participant ? name(participant) : "VĐV"} khỏi đội`} title="Bỏ khỏi đội"><Trash2 size={16}/></button></form></div>; })}{!entryMembers.length && <p className="roster-empty">Đội/cặp này chưa có VĐV. Dùng biểu mẫu phía trên để gán.</p>}</div></div>
