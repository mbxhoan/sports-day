...
import type { Entry, Fixture, FixtureEntry, FixtureSlot, Group, GroupEntry, Organization, Standing, Tournament } from "@/lib/site";
import { applySportExcelImport, confirmFixtureReset, confirmGroupStandings, deleteMedia, prepareSportExcelImport, previewFixtureReset, resetSportResults, rollbackSportExcelImport, saveFixtureResult, saveFixtureSlot, saveManualStandings, saveRaceResult, saveRecordAction, saveRosterRecord, saveScoringRule, saveSportGalleryDriveUrl, setArchived, uploadMedia, uploadSportIcon } from "../../actions";
...
function selectColumns(entity: AdminEntity) { const extra: Partial<Record<AdminEntity, string[]>> = { sports: ["gallery_drive_url"], tournaments: ["competition_mode", "source_metadata", "scoring_rule"], groups: ["standings_confirmed_at"], fixtures: ["source_code"], fixture_entries: ["result_status"] }; return [...new Set(["id", "archived_at", ...adminEntities[entity].fields.map((field) => field.name), ...(extra[entity] ?? [])])].join(","); }
...
function ManualStandingsEditor({ tournament, entries, standings, groupId, organizations = [] }: { tournament: Row; entries: Row[]; standings: Row[]; groupId?: string; organizations?: Row[] }) {
  const key = groupId ?? "";
  ...
  return <section className="admin-card manual-standings-editor"><h3>{summary(tournament)}</h3><p className="results-help">{race ? "Nhập hạng, thành tích và trạng thái; đơn vị hiển thị cùng tên vận động viên/đội." : "Nhập các chỉ số khi không có bracket; Điểm tự tính theo rule nếu hạng mục có rule."}</p><AdminRecordForm action={saveManualStandings}><input type="hidden" name="tournament_id" value={String(tournament.id)}/>{groupId && <input type="hidden" name="group_id" value={groupId}/>}<input type="hidden" name="manual_mode" value={race ? "race" : "standings"}/><div className="manual-standings-list">{entries.filter((entry) => !entry.archived_at).map((entry) => { const row = byEntry.get(String(entry.id)); const organization = organizations.find((item) => String(item.id) === String(entry.organization_id)); const difference = standingDifference({ score_for: Number(row?.score_for ?? 0), score_against: Number(row?.score_against ?? 0) }); return <div className="manual-standings-row" key={entry.id}><input type="hidden" name="entry_id" value={String(entry.id)}/><span><b>{summary(entry)}</b>{organization && <small>{summary(organization, "organizations")}</small>}</span>{race ? <><label>Hạng<input name="rank" type="number" min="1" defaultValue={String(row?.rank ?? "")}/></label><label>Thành tích<input name="score" defaultValue={String(row?.score ?? "")}/></label><label>Trạng thái<input name="result_status" defaultValue={String(row?.result_status ?? "")}/></label></> : <><label>Hạng<output className="auto-rank-cell">{String(row?.rank ?? "Tự động")}</output></label><label>Trận<input name="played" type="number" min="0" defaultValue={String(row?.played ?? 0)}/></label><label>Thắng<input name="won" type="number" min="0" defaultValue={String(row?.won ?? 0)}/></label><label>Hòa<input name="drawn" type="number" min="0" defaultValue={String(row?.drawn ?? 0)}/></label><label>Thua<input name="lost" type="number" min="0" defaultValue={String(row?.lost ?? 0)}/></label><label>Điểm ghi<input name="score_for" type="number" min="0" step="any" defaultValue={String(row?.score_for ?? 0)}/></label><label>Điểm thua<input name="score_against" type="number" min="0" step="any" defaultValue={String(row?.score_against ?? 0)}/></label><label>Điểm<input name="points" type="number" min="0" step="any" defaultValue={String(row?.points ?? 0)}/></label><output>+/- {difference}</output></>}</div>; })}</div><SubmitButton className="gold-button"><Save size={15}/>{race ? "Lưu kết quả / Save results" : "Lưu bảng / Save standings"}</SubmitButton></AdminRecordForm>{groupId && !race && <AdminRecordForm action={confirmGroupStandings} className="confirm-standings"><input type="hidden" name="group_id" value={groupId}/><SubmitButton className="gold-button">Xác nhận bảng / Confirm group</SubmitButton></AdminRecordForm>}</section>;
}

function RaceResultsEditor({ tournament, fixture, entries, fixtureEntries, organizations = [] }: { tournament: Row; fixture: Row; entries: Row[]; fixtureEntries: Row[]; organizations?: Row[] }) {
  ...
  return <section className="admin-card manual-standings-editor"><h3>{summary(tournament)} · {round}</h3><p className="results-help">Nhập hạng, thành tích và trạng thái cho lượt này.</p><AdminRecordForm action={saveRaceResult}><input type="hidden" name="fixture_id" value={String(fixture.id)}/><div className="race-result-list">{fixtureEntries.map((row) => { const entry = entriesById.get(String(row.entry_id)); if (!entry) return null; const organization = organizations.find((item) => String(item.id) === String(entry.organization_id)); return <div key={row.id}><input type="hidden" name="entry_id" value={String(row.entry_id)}/><span><b>{summary(entry)}</b>{organization && <small>{summary(organization, "organizations")}</small>}</span><label>Hạng<input name="rank" type="number" min="1" defaultValue={String(row.rank ?? "")}/></label><label>Thành tích<input name="score" defaultValue={String(row.score ?? row.score_numeric ?? "")}/></label><label>Trạng thái<SearchableSelect name="result_status" value={String(row.result_status ?? "")} options={[{ value: "finished", label: "Hoàn thành" }, { value: "dns", label: "DNS" }, { value: "dnf", label: "DNF" }, { value: "dsq", label: "DSQ" }]} placeholder="Đang cập nhật"/></label></div>; })}</div><SubmitButton className="gold-button"><Save size={15}/>Lưu kết quả / Save results</SubmitButton></AdminRecordForm></section>;
}
...
export default async function SportAdminPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ edit?: string; section?: string; tournament?: string; reset?: string; affected?: string; reset_sport?: string; reset_done?: string; reset_error?: string; import?: string; error?: string; q?: string; page?: string }> }) {
  ...
  const canResetSportResults = tenantSlug === "ptsc2026" && tenantId === "22222222-2222-2222-2222-222222222222";
  let sportResetPreview: { fixtures: number; fixture_entries: number; standings: number; confirmed_groups: number; awards: number } | null = null;
  let sportResetPreviewError = false;
  if (section === "results" && canResetSportResults && requested.reset_sport === sport.id) {
    const { data, error } = await supabase.rpc("reset_sport_results_and_awards", { p_sport_id: sport.id, p_confirm: false });
    if (error || !data || typeof data !== "object" || Array.isArray(data) || data.sport_id !== sport.id) {
      sportResetPreviewError = true;
    } else {
      const counts = [data.fixtures, data.fixture_entries, data.standings, data.confirmed_groups, data.awards].map(Number);
      if (counts.some((count) => !Number.isSafeInteger(count) || count < 0)) sportResetPreviewError = true;
      else sportResetPreview = { fixtures: counts[0], fixture_entries: counts[1], standings: counts[2], confirmed_groups: counts[3], awards: counts[4] };
    }
  }
  const manual = isManualSport(String(sport.slug));
  const tournamentsResult = await supabase.from("tournaments").select(selectColumns("tournaments")).eq("tenant_id", tenantId).eq("sport_id", sport.id).order("archived_at", { ascending: true, nullsFirst: true }).order("sort_order").range(0, 999);
  ...
  const searchText = String(requested.q ?? "").trim().slice(0, 80);
  const pageOffset = Math.max(0, Number.parseInt(String(requested.page ?? "0"), 10) || 0) * 250;
  const all = (entity: AdminEntity, search = "") => {
    const query = supabase.from(entity).select(selectColumns(entity)).eq("tenant_id", tenantId).order("archived_at", { ascending: true, nullsFirst: true });
    if (search && entity === "participants") query.ilike("full_name", `%${search}%`);
    if (search && entity === "entries") query.ilike("name_vi", `%${search}%`);
    return query.range(pageOffset, pageOffset + 249);
  };
  const scoped = (entity: AdminEntity, column: string, ids: string[], search = "") => { const query = all(entity, search); return ids.length ? query.in(column, ids) : query.limit(0); };
  ...
  if (section === "teams") {
    const entriesResult = await scoped("entries", "tournament_id", tournamentIds, searchText);
    rows.entries = asRows(entriesResult.data);
    const entryIds = new Set(rows.entries.map((item) => item.id));
    const [organizationsResult, entryMembersResult, participantsResult] = await Promise.all([all("organizations"), scoped("entry_members", "entry_id", [...entryIds]), all("participants", searchText)]);
    rows.organizations = asRows(organizationsResult.data);
    rows.entry_members = asRows(entryMembersResult.data);
    rows.participants = asRows(participantsResult.data);
  } else if (section === "schedule") {
    const [groupsResult, fixturesResult, entriesResult] = await Promise.all([scoped("groups", "tournament_id", tournamentIds), scoped("fixtures", "tournament_id", tournamentIds), scoped("entries", "tournament_id", tournamentIds)]);
    rows.groups = asRows(groupsResult.data);
    rows.fixtures = asRows(fixturesResult.data);
    rows.entries = asRows(entriesResult.data);
    const groupIds = new Set(rows.groups.map((item) => item.id));
    const fixtureIds = new Set(rows.fixtures.map((item) => item.id));
    const [groupEntriesResult, fixtureEntriesResult, venuesResult, courtsResult] = await Promise.all([scoped("group_entries", "group_id", [...groupIds]), scoped("fixture_entries", "fixture_id", [...fixtureIds]), all("venues"), all("courts")]);
    ...
    try {
      const [entriesResult, fixturesResult, standingsResult, awardsResult, groupsResult, organizationsResult] = await withTimeout(() => Promise.all([scoped("entries", "tournament_id", tournamentIds), scoped("fixtures", "tournament_id", tournamentIds), scoped("standings", "tournament_id", tournamentIds), scoped("awards", "tournament_id", tournamentIds), scoped("groups", "tournament_id", tournamentIds), all("organizations")]), 10_000);
      if ([entriesResult, fixturesResult, standingsResult, awardsResult, groupsResult, organizationsResult].some((result) => result.error)) throw new Error("Results query failed");
      rows.entries = asRows(entriesResult.data);
      rows.fixtures = asRows(fixturesResult.data);
      rows.standings = asRows(standingsResult.data);
      rows.awards = asRows(awardsResult.data);
      rows.groups = asRows(groupsResult.data);
      rows.organizations = asRows(organizationsResult.data);
      const resultEntryIds = new Set(rows.entries.map((item) => item.id));
      const entryMembersResult = await scoped("entry_members", "entry_id", [...resultEntryIds]);
      if (entryMembersResult.error) throw new Error("Results query failed");
      rows.entry_members = asRows(entryMembersResult.data);
      const resultParticipantIds = new Set(rows.entry_members.map((item) => String(item.participant_id)));
      const participantsResult = await scoped("participants", "id", [...resultParticipantIds]);
      if (participantsResult.error) throw new Error("Results query failed");
      rows.participants = asRows(participantsResult.data);
      ...
      if (fixtureIds.length) fixtureSlotsQuery.in("fixture_id", fixtureIds);
      else fixtureSlotsQuery.limit(0);
      const groupEntriesQuery = scoped("group_entries", "group_id", groupIds);
      const [fixtureEntriesResult, groupEntriesResult, fixtureSlotsResult] = await withTimeout(() => Promise.all([activeFixtureEntriesQuery, groupEntriesQuery, fixtureSlotsQuery]), 10_000);
    ...
    rows.media = asRows((await supabase.from("media").select(selectColumns("media")).eq("tenant_id", tenantId).eq("sport_id", sport.id).eq("kind", "gallery").is("archived_at", null).order("sort_order").range(0, 999)).data);
  ...
  return <main className="admin-page"><header className="admin-header"><div><b>{summary(sport)}</b><small>Quản lý môn / Sport operations</small></div><Link href="/admin"><ArrowLeft size={16}/> Dashboard</Link></header><div className="admin-main admin-sport-main"><div className="admin-sport-workspace">
    ...
    <div className="admin-sport-panels"><section id="overview" className="admin-card"><div className="sport-admin-hero"><div><span className="admin-kicker">THÔNG TIN MÔN</span><h1>{summary(sport)}</h1><p>Cập nhật tên, mô tả, thể lệ và hình ảnh đại diện. Các thay đổi này chỉ áp dụng cho môn đang chọn.</p></div><div className="sport-logo-editor"><div className="sport-logo-preview">{String(sport.emoji ?? "").startsWith("http") || String(sport.emoji ?? "").startsWith("/") ? <Image src={String(sport.emoji)} alt={`Logo ${summary(sport)}`} fill sizes="72px"/> : <span>{String(sport.emoji ?? "") || "—"}</span>}</div><form action={uploadSportIcon}><input type="hidden" name="sport_id" value={sport.id}/><label><span>Logo môn thể thao</span><input name="file" type="file" accept="image/png,image/jpeg,image/webp" required/></label><button className="gold-button"><Save size={15}/>Đổi logo</button></form></div></div><AdminRecordForm action={saveRecordAction}><input type="hidden" name="entity" value="sports"/><input type="hidden" name="id" value={sport.id}/><input type="hidden" name="sport_slug" value={slug}/><Fields fields={adminEntities.sports.fields} row={sport} rows={scopedRows}/><button className="gold-button"><Save size={15}/>Lưu thông tin môn</button></AdminRecordForm></section>
      ...
      <section id="categories"><CrudSection entity="tournaments" rows={scopedRows.tournaments} allRows={scopedRows} section="categories" editTarget={editTarget} sportSlug={slug}/><div className="admin-card"><h2>Quy tắc tính BXH / Standings rules</h2><p className="results-help">Môn đối kháng dùng bracket: nhập tỷ số trên bracket, BXH tự tính Trận/Thắng/Hòa/Thua/Ghi/Thua/+/-/Điểm theo rule. Chỉ để Manual khi điều lệ không tính được từ từng trận.</p><div className="record-list">{scopedRows.tournaments.map((tournament) => { const item = rule(tournament); return <details className="record" key={tournament.id}><summary>{summary(tournament)}</summary><form action={saveScoringRule}><input type="hidden" name="tournament_id" value={tournament.id}/><div className="admin-fields"><label><span>Loại / Type</span><SearchableSelect name="scoring_type" value={item.type ?? "manual"} options={[{ value: "manual", label: "Manual / Other PDF rule" }, { value: "head-to-head", label: "Head-to-head points" }]} placeholder="Chọn / Select"/></label><label><span>Thắng / Win</span><input name="win_points" type="number" step="any" defaultValue={item.win ?? ""}/></label><label><span>Hòa / Draw</span><input name="draw_points" type="number" step="any" defaultValue={item.draw ?? ""}/></label><label><span>Thua / Loss</span><input name="loss_points" type="number" step="any" defaultValue={item.loss ?? ""}/></label></div><button className="gold-button"><Save size={15}/>Lưu quy tắc / Save rule</button></form></details>; })}</div></div></section>
      ...
      <section id="results" className="admin-group">{requested.error && section === "results" && <p className="results-error" role="alert">{requested.error}</p>}{canResetSportResults && <div className="admin-card reset-warning"><h2>Reset kết quả và huy chương môn đang chọn</h2><p>Chỉ ảnh hưởng môn <strong>{summary(sport)}</strong>: xóa điểm, thứ hạng và ghi chú kết quả trận; đưa trận đang diễn ra/đã hoàn tất về lịch chờ. Các huy chương/giải thưởng gắn với môn, hạng mục, đội hoặc VĐV được chuyển vào lưu trữ; bản ghi vẫn có thể khôi phục. Tổng huy chương BXH đoàn toàn sự kiện được giữ nguyên vì không tách theo môn. Lịch thi đấu, cấu trúc nhánh và đội/VĐV được giữ lại.</p>{requested.reset_done === "1" && <p role="status">Đã reset kết quả và lưu trữ huy chương môn {summary(sport)}.</p>}{requested.reset_error && requested.reset_sport === sport.id && <p className="results-error" role="alert">{requested.reset_error}</p>}{sportResetPreviewError && <p className="results-error" role="alert">Không tải được bản xem trước reset. Dữ liệu chưa bị thay đổi.</p>}{sportResetPreview ? <><p>Kết quả xem trước: {sportResetPreview.fixtures} trận, {sportResetPreview.fixture_entries} dòng đội trong trận, {sportResetPreview.standings} dòng bảng xếp hạng, {sportResetPreview.confirmed_groups} bảng đã xác nhận và {sportResetPreview.awards} huy chương/giải thưởng sẽ được lưu trữ. Đội/VĐV vòng sau được đồng bộ theo nguồn còn hiệu lực.</p><form action={resetSportResults}><input type="hidden" name="sport_id" value={sport.id}/><input type="hidden" name="sport_slug" value={slug}/><label><input type="checkbox" name="confirm" value="yes" required/> Tôi đã xem phạm vi trên và xác nhận reset kết quả cùng danh sách huy chương của môn này.</label><SubmitButton className="archive-button">Xác nhận reset môn</SubmitButton></form></> : !requested.reset_error && !sportResetPreviewError && <Link className="ghost-button" href={`/admin/sports/${encodeURIComponent(slug)}?section=results&reset_sport=${encodeURIComponent(sport.id)}#results`}>Xem trước phạm vi reset</Link>}</div>}{resetTarget && <div className="admin-card reset-warning"><h2>⚠ Mở lại vòng sau</h2><p>Thao tác sẽ xoá tỷ số, thứ hạng và người thắng của {resetAffected} trận phụ thuộc. Không thể hoàn tác tự động.</p><form action={confirmFixtureReset}><input type="hidden" name="fixture_id" value={resetTarget}/><input type="hidden" name="sport_slug" value={slug}/><label><input type="checkbox" name="confirm" value="yes" required/> Tôi xác nhận đặt lại dữ liệu vòng sau.</label><SubmitButton className="archive-button">Xác nhận reset</SubmitButton></form></div>}
        ...
        {manual && <div className="manual-standings-stack">{scopedRows.tournaments.map((tournament) => { const tournamentEntries = scopedRows.entries.filter((entry) => entry.tournament_id === tournament.id); const tournamentOrganizations = scopedRows.organizations; if (tournament.competition_mode !== "race") return <ManualStandingsEditor key={tournament.id} tournament={tournament} entries={tournamentEntries} standings={scopedRows.standings.filter((row) => row.tournament_id === tournament.id)} organizations={tournamentOrganizations}/>; const raceGroupId = scopedRows.groups.find((group) => group.tournament_id === tournament.id)?.id; const fixturesForTournament = scopedRows.fixtures.filter((fixture) => fixture.tournament_id === tournament.id && !fixture.archived_at); const sourceFixtures = fixturesForTournament.filter((fixture) => String(fixture.source_code ?? "").startsWith(String(tournament.slug ?? "").toUpperCase())).sort((a, b) => String(a.source_code ?? "").localeCompare(String(b.source_code ?? ""))); const resultFixtures = sourceFixtures.length ? sourceFixtures : fixturesForTournament; const sections = resultFixtures.map((fixture) => ({ fixture, fixtureEntries: fixtureEntriesByFixture.get(String(fixture.id)) ?? [] })).filter((section) => section.fixtureEntries.length); return sections.length ? sections.map(({ fixture, fixtureEntries }) => { const entryIds = new Set(fixtureEntries.map((row) => String(row.entry_id))); return <RaceResultsEditor key={fixture.id} tournament={tournament} fixture={fixture} entries={tournamentEntries.filter((entry) => entryIds.has(String(entry.id)))} fixtureEntries={fixtureEntries} organizations={tournamentOrganizations}/>; }) : <ManualStandingsEditor key={tournament.id} tournament={tournament} entries={tournamentEntries} standings={scopedRows.standings.filter((row) => row.tournament_id === tournament.id)} groupId={raceGroupId} organizations={tournamentOrganizations}/>; })}</div>}
      ...
      <section id="gallery" className="admin-card sport-gallery-admin"><h2>Thư viện ảnh / Gallery</h2><p className="upload-help">Ảnh upload được lưu trong kho quản trị; website công khai mở thư viện Google Drive riêng của môn.</p><form action={saveSportGalleryDriveUrl} className="sport-gallery-drive-form"><input type="hidden" name="sport_id" value={sport.id}/><label><span>Google Drive folder của môn (HTTPS)</span><input name="gallery_drive_url" type="url" placeholder="https://drive.google.com/drive/folders/..." defaultValue={String(sport.gallery_drive_url ?? "")}/></label><div className="admin-actions"><button className="gold-button"><Save size={15}/>Lưu link Drive</button>{String(sport.gallery_drive_url ?? "") && <a className="ghost-button" href={String(sport.gallery_drive_url)} target="_blank" rel="noreferrer">Mở thư viện ↗</a>}</div></form><MediaUploadForm action={uploadMedia} sportId={sport.id} multiple albumFields filterTag={slug}/><MediaGrid rows={scopedRows.media} allRows={scopedRows} editTarget={editTarget} publicUrl={(path) => supabase.storage.from("event-media").getPublicUrl(path).data.publicUrl}/></section>
      <ExcelAdmin slug={slug} imports={excelImports} selected={selectedExcelImport} error={section === "excel" ? requested.error : undefined}/>
    </div></div></div></main>;
}
