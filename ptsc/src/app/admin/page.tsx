import { Archive, ChevronDown, LogOut, RotateCcw, Save, Settings, Trophy } from "lucide-react";
...
import { logout, resetPtscResults, saveEvent, saveGalleryDriveUrl, saveManualLeaderboard, saveOrganizationName, saveRecordAction, setArchived, setOrganizationLeaderboardVisibility, uploadHero } from "./actions";
...
const dashboardColumns = {
  ...
  sports: "id,slug,name_vi,name_en,emoji,description_vi,description_en,rules_vi,rules_en,sort_order,gallery_drive_url,archived_at",
  organizations: "id,code,short_name,name_vi,name_en,logo_path,sort_order,leaderboard_rank,leaderboard_hidden,gold_medals,silver_medals,bronze_medals,archived_at",
...
export default async function AdminPage() {
  ...
  const activeSports = rows.sports.filter((sport) => !sport.archived_at).sort((a, b) => Number(a.sort_order ?? 0) - Number(b.sort_order ?? 0));

  return <main className="admin-page"><header className="admin-header"><div><Trophy/><span><b>PTSC 2026</b><small>Admin / Quản trị</small></span></div><form action={logout}><SubmitButton><LogOut size={16}/>Đăng xuất</SubmitButton></form></header><div className="admin-layout"><aside><a href="#content">Nội dung / Content</a><a href="#organizations">Đơn vị / Organizations</a><a href="#media">Hình ảnh / Media</a><details className="admin-side-menu" open><summary>Môn thể thao / Sports <ChevronDown size={15}/></summary><div>{activeSports.map((sport) => <Link href={`/admin/sports/${sport.slug}?section=overview#overview`} key={sport.id}>{summary(sport)}</Link>)}</div></details><a href="#data">Dữ liệu thi đấu / Competition</a><Link href="/admin/excel">Import template PTSC</Link></aside><div className="admin-main">
    <div className="admin-title"><div><h1>Dashboard</h1><p>Xin chào, {admin.display_name}</p></div><a href="/" target="_blank">Xem website ↗</a></div>
    <section id="sports" className="admin-quick-sports"><h2 className="section-label">Môn thể thao / Sports</h2><div className="admin-sport-grid">{activeSports.map((sport) => <Link key={sport.id} href={`/admin/sports/${sport.slug}`}><b>{summary(sport)}</b><small>Hạng mục, đội, lịch, kết quả, thư viện</small></Link>)}</div></section>
    <section id="content" className="admin-card"><h2>Nội dung sự kiện / Event content</h2><form action={saveEvent}><div className="admin-fields"><label><span>Tên sự kiện VI</span><input name="event_name_vi" defaultValue={String(event.event_name_vi ?? "")}/></label><label><span>Event name EN</span><input name="event_name_en" defaultValue={String(event.event_name_en ?? "")}/></label><label><span>Phụ đề VI</span><input name="subtitle_vi" defaultValue={String(event.subtitle_vi ?? "")}/></label><label><span>Subtitle EN</span><input name="subtitle_en" defaultValue={String(event.subtitle_en ?? "")}/></label><label><span>Giới thiệu VI</span><MarkdownInput name="about_vi" defaultValue={String(event.about_vi ?? "")}/></label><label><span>About EN</span><MarkdownInput name="about_en" defaultValue={String(event.about_en ?? "")}/></label><label><span>Địa điểm VI</span><input name="venue_vi" defaultValue={String(event.venue_vi ?? "")}/></label><label><span>Venue EN</span><input name="venue_en" defaultValue={String(event.venue_en ?? "")}/></label><label><span>Bắt đầu</span><input type="datetime-local" name="start_at" defaultValue={event.start_at ? toVietnamLocalInput(String(event.start_at)) : ""}/></label><label><span>Kết thúc</span><input type="datetime-local" name="end_at" defaultValue={event.end_at ? toVietnamLocalInput(String(event.end_at)) : ""}/></label></div><SubmitButton className="gold-button"><Save size={15}/>Lưu nội dung / Save</SubmitButton></form></section>
    <section id="media" className="admin-card admin-media-hub"><div className="admin-title"><div><h2>Hình ảnh / Media</h2><p>Quản lý KV PTSC và thư viện Drive. Ảnh upload theo môn chỉ phục vụ kho quản trị.</p></div><span className="readonly-badge">Public mở Drive</span></div><form action={saveGalleryDriveUrl} className="admin-media-drive-form"><label><span>Google Drive folder toàn sự kiện (HTTPS)</span><input name="gallery_drive_url" type="url" placeholder="https://drive.google.com/drive/folders/..." defaultValue={String(event.gallery_drive_url ?? "")}/></label><SubmitButton className="gold-button"><Save size={15}/>Lưu link Drive</SubmitButton></form><div className="hero-upload-grid">{([['desktop','KV desktop',String(event.hero_path || '/kv-ptsc-placeholder.png')],['mobile','KV mobile',String(event.hero_mobile_path || '/kv-ptsc-mobile-placeholder.png')]] as const).map(([variant,label,path]) => <form action={uploadHero} key={variant} className="hero-upload"><b>{label}</b><div className="hero-upload-preview"><Image src={path} alt={label} fill sizes="320px" unoptimized={path.startsWith("http")}/></div><input type="hidden" name="variant" value={variant}/><input type="file" name="file" accept="image/png,image/jpeg,image/webp" required/><SubmitButton className="gold-button">Tải lên / Upload</SubmitButton></form>)}</div><div className="admin-media-sport-links"><div><h3>Thư viện ảnh theo môn</h3><p className="upload-help">Mở từng môn để upload nhiều ảnh, chỉnh metadata hoặc lưu trữ ảnh.</p></div><div className="admin-media-sport-grid">{activeSports.map((sport) => <Link href={`/admin/sports/${sport.slug}?section=gallery#gallery`} key={sport.id}><b>{summary(sport)}</b><span>Quản lý thư viện →</span></Link>)}</div></div></section>
    <section id="organizations" className="admin-card organization-admin-card"><div className="organization-admin-heading"><div><h2>Danh mục đơn vị</h2><p className="upload-help">Có thể sửa tên đầy đủ và tên viết tắt hiển thị. Mã kỹ thuật được giữ nguyên để không hỏng liên kết dữ liệu.</p></div><span className="readonly-badge">Mã kỹ thuật cố định</span></div><div className="organization-editor-list">{rows.organizations.filter((row) => !row.archived_at).map((row) => <div className="organization-editor-row" key={row.id}><div className="organization-identity"><span>{String(row.code ?? "—")}</span><div><b>{summary(row)}</b><small>Website dùng tên viết tắt trong bảng đấu</small></div></div><div className="organization-controls"><form action={saveOrganizationName} className="organization-name-form"><input type="hidden" name="id" value={row.id}/><label><span>Tên đầy đủ</span><input name="name_vi" defaultValue={String(row.name_vi ?? "")} maxLength={200} required/></label><label><span>Tên viết tắt hiển thị</span><input name="short_name" defaultValue={String(row.short_name ?? row.code ?? "")} maxLength={160} required/></label><SubmitButton className="gold-button"><Save size={15}/>Lưu đơn vị</SubmitButton></form><form action={setOrganizationLeaderboardVisibility} className="organization-visibility-form"><input type="hidden" name="id" value={row.id}/><input type="hidden" name="hidden" value={row.leaderboard_hidden ? "false" : "true"}/>{row.leaderboard_hidden && <small className="organization-hidden-label">Đang ẩn khỏi BXH công khai</small>}<SubmitButton className="organization-visibility-button">{row.leaderboard_hidden ? "Hiện trên BXH" : "Ẩn khỏi BXH"}</SubmitButton></form></div></div>)}</div></section>
    ...
    <section id="reset-results" className="admin-card reset-warning"><h2>Reset kết quả test PTSC</h2><p>Chỉ xoá tỷ số, trạng thái kết quả, người thắng và bảng điểm hiện tại của PTSC. Lịch, nhánh đấu, đội/VĐV và dữ liệu legacy được giữ nguyên.</p><form action={resetPtscResults}><label><input type="checkbox" name="confirm" value="yes" required/> Tôi đã dừng nhập kết quả và xác nhận reset toàn bộ kết quả test PTSC.</label><SubmitButton className="archive-button">Xác nhận reset kết quả</SubmitButton></form></section>
