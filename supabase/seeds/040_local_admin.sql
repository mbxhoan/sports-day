...
on conflict (tenant_id, user_id) do update set display_name = excluded.display_name, archived_at = null;
...
insert into auth.users (
  ...
  'authenticated', 'authenticated', 'admin@ptsc.vn',
  extensions.crypt('PTSC2026!', extensions.gen_salt('bf')),
  now(), '{"provider":"email","providers":["email"]}', '{}', now(), now(), '', '', '', ''
) on conflict (id) do update set
  encrypted_password = excluded.encrypted_password,
  email_confirmed_at = excluded.email_confirmed_at,
...
insert into auth.identities (
  ...
  'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb',
  'admin@ptsc.vn',
  '{"sub":"bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb","email":"admin@ptsc.vn","email_verified":true}'::jsonb,
  'email', now(), now(), now()
) on conflict (provider_id, provider) do update set identity_data = excluded.identity_data, updated_at = now();

insert into public.admin_users (tenant_id, user_id, display_name)
values ('22222222-2222-2222-2222-222222222222', 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'PTSC Admin')
on conflict (tenant_id, user_id) do update set display_name = excluded.display_name, archived_at = null;
